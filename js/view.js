export function render(data, htmlElement) {
  ...
  const weatherInfo = `
            <div class="${section}">
                ...
                <p>Поточна температура: ${data.main.temp}&#186;С (відчуваєтся як: ${data.main.feels_like}&#186;С)</p>
                <p>Тиск: ${Math.round(Number(data.main.pressure) * 0.7501)} мм рт. ст.</p>
            </div>
            `;

  document.getElementById(htmlElement.id).innerHTML = weatherInfo;
}
